...
function createStackedPlanesBufferGeometry(
  ...
  for (let i = 0; i < n; i++) {
    const xOff = xOffsetBase + i * (width + spacing);
    const uvXOff = Math.random() * 300;
    const uvYOff = Math.random() * 300;
    for (let j = 0; j <= heightSegments; j++) {
      const y = height * (j / heightSegments - 0.5);
      positions.set([xOff, y, 0, xOff + width, y, 0], vertexOffset * 3);
      const uvY = j / heightSegments;
      uvs.set([uvXOff, uvY + uvYOff, uvXOff + 1, uvY + uvYOff], uvOffset);
      if (j < heightSegments) {
